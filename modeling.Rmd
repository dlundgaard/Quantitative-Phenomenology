...
```{r}
# fit <- brm(
#   formula = bf(OBN ~ 0 + substance + gender + age_at_experience + years_since_experience + (1 | author)) + set_rescor(FALSE),
#   family = gaussian(),
#   # prior = c(
...
fit <- brm(
  formula = (
    bf(OBN ~ 0 + substance + gender + age_at_experience + years_since_experience + (1 | author)) +
    bf(AED ~ 0 + substance + gender + age_at_experience + years_since_experience + (1 | author)) +
    bf(VIR ~ 0 + substance + gender + age_at_experience + years_since_experience + (1 | author))
  ) + set_rescor(TRUE),
  family = gaussian,
  # data = data,
  data = completed,
...
for (focused_dimension in c("OBN", "AED", "VIR")) {
  ...
  fit <- brm(
    formula = bf(focused_dimension ~ 0 + substance + gender + age_at_experience + years_since_experience + (1 | author)),
    family = gaussian,
    data = data,
    backend = "cmdstanr",
    iter = 2000,
